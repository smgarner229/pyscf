...
def kernel(mp,  mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2,
    ...
    for it in range(niter):        
        ...
        #mo_energy[nocc_inact:nocc_inact+nact], U = scipy.linalg.eigh(fock)
        #mo_coeff_act = numpy.matmul(mo_coeff_act, U)
        #mp.mo_energy = mo_energy
        #mp.mo_coeff[nocc_inact:nocc_inact+nact,nocc_inact:nocc_inact+nact]  = mo_coeff_act
...
def make_veff(mp):
    ...
    h2mo_ggoo_act = h2mo_ggoo[nocc_inact:nocc_inact+nact, nocc_inact:nocc_inact+nact, nocc_inact:, nocc_inact:]

    h2mo_goog = ao2mo.general(mp._scf._eri, (cg,co,co,cg))
    h2mo_goog = h2mo_goog.reshape(nmo,nocc,nocc,nmo)
    h2mo_goog_act = h2mo_goog[nocc_inact:nocc_inact+nact, nocc_inact:, nocc_inact:, nocc_inact:nocc_inact+nact]
...
def make_amp(mp):
    ...
    h2mo_act = h2mo[nocc_inact:, :nvir_act, nocc_inact:, :nvir_act]
...
def first_BCH(mp, fock_hf, tmp1, tmp1_bar, c0):
    ...
    h2mo_act = h2mo_ovov[nocc_inact:, :nvir_act, nocc_inact:, :nvir_act]

    h2mo_ovgv = ao2mo.general(mp._scf._eri, (co,cv,cg,cv))
    h2mo_ovgv = h2mo_ovgv.reshape(nocc,nvir,nmo,nvir)
    h2mo_ovgv_act = h2mo_ovgv[nocc_inact:, :nvir_act, nocc_inact:nocc_inact+nact, :nvir_act]

    h2mo_ovog = ao2mo.general(mp._scf._eri, (co,cv,co,cg))
    h2mo_ovog = h2mo_ovog.reshape(nocc,nvir,nocc,nmo)
    h2mo_ovog_act = h2mo_ovog[nocc_inact:, :nvir_act, nocc_inact:, nocc_inact:nocc_inact+nact]
    
    c0 -= 4.*numpy.sum(h2mo_act * tmp1_bar)
    ####################### c1[j,B] #########################
...
def second_BCH(mp, fock_hf, tmp1, tmp1_bar, c0):
    ...
    y1 = numpy.einsum('ijkl -> kl', numpy.einsum('ijkl -> klij',numpy.tile(fock_hf[:nocc_act,nocc_act:],(nocc_act,nvir_act,1,1))) * tmp1_bar)

    c1[:nocc_act,nocc_act:] += 4.*numpy.einsum('ijkl -> ij',numpy.tile(y1,(nocc_act,nvir_act,1,1)) * tmp1_bar)
...
# don't modify the following attributes, they are not input options
    ...
    def kernel(self, shift=0.0, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2,
        ...
        self.e_corr = _kern(self, mo_energy, mo_coeff,
