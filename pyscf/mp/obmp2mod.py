...
def kernel(mp,  mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2,
    ...
    for it in range(niter):
        ...
        fock_hf += + 2.*numpy.einsum('ijkk -> ij',h2mo[:,:,:nocc,:nocc]) - numpy.einsum('ijjk -> ik',h2mo[:,:nocc,:nocc,:])          
...
def make_amp(mp, h2mo):
    ...
    tmp1 = mp.ampf * h2mo[:nocc,nocc:,:nocc,nocc:]/x
    #print('Hello', mo_energy.shape)             
...
def first_BCH(mp, fock_hf, tmp1, tmp1_bar, h2mo, c0):
    ...
    c0 -= 4.*numpy.sum(numpy.multiply(h2mo[:nocc,nocc:,:nocc,nocc:],tmp1_bar))
    ...
    h2mo_cut = numpy.einsum('ijkl -> ijlk',h2mo[:nocc,nocc:,:,nocc:]).reshape(-1,nmo)
    for i in range(nocc):
    ...
    h2mo_cut2 = h2mo[:nocc,nocc:,:nocc,:].reshape(-1,nmo)
    for i in range(nocc,nmo):
        mul2 = numpy.multiply(h2mo_cut2,numpy.repeat(tmp1_bar[:,:,:,i-nocc],nmo,2).reshape(-1,nmo))
        c1[:,i] = -4*numpy.sum(mul2,0)
